...
func (r *exchangeRateRepository) Create(ctx context.Context, rate *dto.ExchangeRateDTO) (*dto.ExchangeRateDTO, error) {
	...
	row := r.db.QueryRowContext(ctx,
		`INSERT INTO exchange_rates (`+exchangeRateColumns+`) VALUES ($1, $2, $3, $4, $5, $6)
		 ON CONFLICT (user_id, currency, effective_from) DO UPDATE SET units_per_usd = excluded.units_per_usd
		 RETURNING id, created_at`,
		rate.ID, rate.UserID, rate.Currency, rate.UnitsPerUSD, rate.EffectiveFrom, rate.CreatedAt)

	var returnedID string
	var createdAt time.Time
	if err := row.Scan(&returnedID, &createdAt); err != nil {
		return nil, fmt.Errorf("postgresql: upsert exchange rate: %w", err)
	}
	rate.ID = returnedID
...
func (r *exchangeRateRepository) ListByUser(ctx context.Context, userID string) ([]*dto.ExchangeRateDTO, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT `+exchangeRateColumns+` FROM exchange_rates WHERE user_id = $1
		 ORDER BY currency ASC, effective_from DESC`, userID)
	if err != nil {
		return nil, fmt.Errorf("postgresql: query exchange rates: %w", err)
	}
	defer rows.Close()
...
func (r *exchangeRateRepository) Delete(ctx context.Context, userID, rateID string) error {
	result, err := r.db.ExecContext(ctx,
		`DELETE FROM exchange_rates WHERE id = $1 AND user_id = $2`, rateID, userID)
	if err != nil {
		return fmt.Errorf("postgresql: delete exchange rate: %w", err)
	}
	n, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("postgresql: delete exchange rate rows affected: %w", err)
	}
	if n == 0 {
...
func scanExchangeRate(row scannable) (*dto.ExchangeRateDTO, error) {
	var rate dto.ExchangeRateDTO
	err := row.Scan(&rate.ID, &rate.UserID, &rate.Currency, &rate.UnitsPerUSD, &rate.EffectiveFrom, &rate.CreatedAt)
	if err != nil {
		return nil, err
	}
	return &rate, nil
