...
func seedSource(t *testing.T, db *sql.DB) {
	...
	if _, err := movementRepo.CreateBatch(ctx, []*dto.MovementDTO{
		{UserID: testUserID, Amount: -5000, Currency: "usd", Description: "transfer out", Category: string(entities.CategoryTransfer), PaymentMethod: "other", AccountID: &account.ID, TransferID: &transferID, Status: string(entities.MovementStatusActive), SyncStatus: string(entities.SyncStatusPending), Timestamp: now, CreatedAt: now},
		{UserID: testUserID, Amount: 5000, Currency: "usd", Description: "transfer in", Category: string(entities.CategoryTransfer), PaymentMethod: "other", TransferID: &transferID, Status: string(entities.MovementStatusActive), SyncStatus: string(entities.SyncStatusPending), Timestamp: now, CreatedAt: now},
	}); err != nil {
