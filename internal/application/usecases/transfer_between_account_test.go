...
func TestCancelTransferCancelsBothLegsPerSyncStatus(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			transferUC := NewTransferBetweenAccounts(movements, accounts, newFakePlanRepo(), newFakeUserSettingsRepo())
...
func TestCancelMovementRejectsDirectSingleLegCancel(t *testing.T) {
	...
	transfer, err := NewTransferBetweenAccounts(movements, accounts, newFakePlanRepo(), newFakeUserSettingsRepo()).Execute(context.Background(), TransferBetweenAccountsInput{
		UserID: "u1", FromAccountID: from.ID, ToAccountID: to.ID, Amount: 500,
	})
	if err != nil {
		t.Fatal(err)
