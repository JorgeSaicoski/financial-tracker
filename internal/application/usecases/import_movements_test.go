...
func newImportFixtures() (*importMovementsUseCase, *fakeMovementRepo, *fakeAccountRepo, *fakeCurrencyRepo) {
	...
	avoidability := 20
	if _, err := categories.Create(context.Background(), &dto.CategoryDTO{Name: "food", AvoidabilityPercent: &avoidability, ContributorIDs: []string{"u1"}}); err != nil {
		panic(err)
	}
	uc := &importMovementsUseCase{movements: movements, accounts: accounts, currencies: currencies, categories: categories}
