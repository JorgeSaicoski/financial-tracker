...
func openTestDB(t *testing.T) *sql.DB {
	...
	if _, err := db.Exec(`TRUNCATE TABLE account_snapshots, movements, credit_card_purchases, accounts, recurring_rules, user_local_archive_settings, user_settings, cards, payment_methods CASCADE`); err != nil {
...
func TestMovementUpdateMetadataAndFinancial(t *testing.T) {
	...
	if err := repo.UpdateMetadata(ctx, created.ID, "renamed", string(entities.CategoryTransport), "pix", &account.ID); err != nil {
	...
	if err := repo.UpdateMetadata(ctx, "missing", "x", string(entities.CategoryOther), "other", nil); !errors.Is(err, apperrors.ErrNotFound) {
