...
func openTestDB(t *testing.T) *sql.DB {
	...
	if _, err := db.Exec(`TRUNCATE TABLE account_snapshots, movements, credit_card_purchases, accounts, recurring_rules, user_local_archive_settings, user_settings, cards, payment_methods, plans, user_data_keys, user_ledger_pseudonyms, subscriptions, categories, category_maintainers, user_hidden_categories CASCADE`); err != nil {
...
func TestMovementUpdateMetadataAndFinancial(t *testing.T) {
	...
	if err := repo.UpdateMetadata(ctx, created.ID, "renamed", &transport.ID, "pix", &account.ID, nil); err != nil {
	...
	if err := repo.UpdateMetadata(ctx, "missing", "x", nil, "other", nil, nil); !errors.Is(err, apperrors.ErrNotFound) {
