...
func openTestDB(t *testing.T) *sql.DB {
	...
	if _, err := db.Exec(`TRUNCATE TABLE account_snapshots, movements, credit_card_purchases, accounts, recurring_rules, user_local_archive_settings, user_settings, cards, payment_methods, plans, user_data_keys, user_ledger_pseudonyms, subscriptions CASCADE`); err != nil {
...
func TestMovementCreateGetRoundtrip(t *testing.T) {
	...
	if got.Amount != -450 || got.Description != "coffee" ||
		got.Category != string(entities.CategoryFood) || got.PaymentMethod != "cash" ||
...
func TestMovementUpdateMetadataAndFinancial(t *testing.T) {
	...
	if err := repo.UpdateMetadata(ctx, created.ID, "renamed", string(entities.CategoryTransport), "pix", &account.ID, nil); err != nil {
	...
	if err := repo.UpdateMetadata(ctx, "missing", "x", string(entities.CategoryOther), "other", nil, nil); !errors.Is(err, apperrors.ErrNotFound) {
...
func TestRecurringRuleUpdatesAndSetActive(t *testing.T) {
	...
	if err := repo.UpdateMetadata(ctx, rule.ID, "new desc", string(entities.CategoryFood), "cash", nil); err != nil {
		t.Fatal(err)
	}
