...
func insertMovement(ctx context.Context, ex execer, m *dto.MovementDTO) error {
	_, err := ex.ExecContext(ctx,
		...
		timeOrNil(m.SyncedAt), m.CreatedAt, strOrNil(m.AccountID), strOrNil(m.TransferID), strOrNil(m.RecurringRuleID))
	if err != nil {
...
func scanMovement(row scannable) (*dto.MovementDTO, error) {
	...
	err := row.Scan(
		...
		&syncedAt, &m.CreatedAt, &accountID, &transferID, &recurringRuleID)
	if err != nil {
		return nil, err
	}
