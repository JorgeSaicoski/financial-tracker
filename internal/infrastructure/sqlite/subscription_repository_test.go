...
func TestSubscriptionUpsertRoundtripAndReplace(t *testing.T) {
	...
	// Resubscribing (or a renewal webhook) replaces the row rather than
	// adding a second one for the same user.
...
func TestSubscriptionListLapsable(t *testing.T) {
	...
	// Past_due, period ended 10 days ago: past a 7-day grace period.
	...
	// Past_due, period ended 2 days ago: still inside the grace period.
	if _, err := repo.Upsert(ctx, &dto.SubscriptionDTO{
	...
	// Active: never a candidate regardless of dates.
	if _, err := repo.Upsert(ctx, &dto.SubscriptionDTO{
