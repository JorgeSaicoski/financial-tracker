...
func TestPlanCreateGetRoundtrip(t *testing.T) {
	...
	// Not owned by this user, and doesn't exist: both ErrNotFound, no
	// distinguishing which (same rule as GetMovementUseCase).
...
func TestPlanCreateSavingsWithAccountAndTarget(t *testing.T) {
	...
	account, err := accounts.Create(ctx, &dto.AccountDTO{
		UserID: planTestUserID, Name: "vacation fund", Type: "bank", Currency: "usd", CreatedAt: time.Now().UTC(),
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	plan := &dto.PlanDTO{
		...
		AccountID: &account.ID, StartDate: time.Now().UTC(), Status: "active",
	}
...
func TestPlanListByUserNewestFirst(t *testing.T) {
	...
	// A different user's plan must never show up in this user's list.
...
func TestPlanUpdateOverwritesFieldsAndRejectsWrongOwner(t *testing.T) {
	...
	newEnd := time.Now().UTC().AddDate(0, 6, 0)
	...
	// Updating a plan owned by someone else (or a missing id) is
