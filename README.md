...
make remove-db  # wipe ALL databases (tracker postgres + ledger postgres) for a fresh start
...
- `ft-postgres` + `financial-tracker` API on `:8081` (BACK-21: the server
  always runs on Postgres, never SQLite — see "Running pieces
  individually" below)
...
1. **financial-tracker API** (works with or without ledger-service up):
   ...
   ```
   Listens on `:8081`, stores data via `DB_DRIVER` (default `postgres`,
   needs `DATABASE_URL=postgres://...`), syncs to `LEDGER_SERVICE_URL`
   ...
   **BACK-21: `DB_DRIVER=sqlite` is never valid here** — the server
   refuses to start with it unless `STANDALONE=true` is also set (see
   "Running locally (standalone)" below). SQLite is exclusively the
   standalone binary's own local file; every server invocation, dev or
   production, auth-enabled or not, uses Postgres.
