...
| `GET` | `/export/movements?currency=&from=&to=` | The revert direction of `POST /import/movements`: every matching active movement, rendered back into the same fixed CSV model (see `internal/infrastructure/csv/README.md`) — a file round-tripped through import then export reproduces the same rows. |
